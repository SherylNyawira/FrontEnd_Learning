$(function() {
    ...
    h = (10 > h) ? "0" + h : h;
    t = (10 > t) ? "0" + t : t;
    var time = (12 > h) ? h + ":" + t + " am" : (h - 12) + ":" + t + " pm";
    ...
    function send() {
        ...
        var like = ["GOOD", "", "", "", "LOL", "ME TOO"];
        var hru = ["HOW R U?", "H R U?", "HOW ARE YOU?", "HRU?", "HRU", "HOW R U", "H R U", "HOW ARE YOU"];
        var good = ["I'M GOOD", "I'M FINE", "I'M FINE U", "I AM FINE", "I'M FINE AND HOW ARE YOU", "MEE TOO", "FINE", "FINE:)", "FINE :)", "MARVELOUS", "AWESOME", "FINE", "GREAT"];
        ...
        if (isInArray(hello, lastmsg)) {
            smsg = "Hello, How are you? ";
            k = "Hello, How are you? ";
        } else if (isInArray(wru, lastmsg)) {
            smsg = "I am goutam";
            k = smsg;
        } else if (isInArray(bad, lastmsg)) {
            smsg = "Thanks for your precious feedback, I'll try to improve that.";
            k = smsg;
        } else if (isInArray(wrud, lastmsg)) {
            k = "Nothing special and you? ";
            smsg = k + "";
        } else if (isInArray(bye, lastmsg)) {
            smsg = "Thanks for checking my code, , Don't forget to like it and please also give your review in comment box..... Bye ";
            k = smsg;
            setTimeout(function() {
                $(".status").html("last seen today at " + time)
            }, 6000);
            ...
            smsg = "Thanks";
            k = smsg;
        } else if (isInArray(s2u, lastmsg)) {
            smsg = "Same to you";
            k = smsg;
        } else if (isInArray(qu, lastmsg)) {
            k = "I don't know ";
            smsg = k + "";
        } else if (isInArray(ge, lastmsg)) {
            k = "Good Evening ";
            smsg = k + "";
        } else if (isInArray(ok, lastmsg)) {
            smsg = "hmm";
            k = smsg;
        } else if (isInArray(gaf, lastmsg)) {
            k = "Good Afternoon";
            smsg = k + "";
        } else if (isInArray(like, lastmsg)) {
            smsg = "<font size='6'></font>";
            k = "ok";
        } else if (isInArray(good, lastmsg)) {
            ...
            smsg = "<b>This are the top results </b><nav class='back'>&larr;</nav><nav class='forword'>&rarr;</nav>";
            k = "This are the Top results";
        } else {
            smsg = "Sorry, I didn't understand, please explain it with proper spellings or If you say so I can search for you. To search, <br> type <q><b> Search Your keyword </b></q> for example type : <b>Search Sololearn</b>";
            k = "Sorry, I didn't understand, please explain it, with proper spellings, or If you say, so I can search for you. To search,type,Search Your keyword,for example, type, Search sololearn ";
        }
